...
export function AiWebStrategyPanel({ clientId }: { clientId: string }) {
  const { state, run, isPending } = useAIAction(() => generateWebStrategyAction(clientId));
  ...
  return (
    <div className="space-y-3 rounded-lg border border-border p-3">
      ...
      {state.status === "idle" && (
        <p className="text-xs text-muted-foreground">Aucune stratégie générée pour l&apos;instant.</p>
      )}
        ...
        <div className="space-y-2 text-sm">
          <div>
            ...
            <ul className="ml-4 list-disc text-muted-foreground">
              {state.data.architecture.map((item, i) => (
                <li key={i}>{item}</li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="ml-4 list-disc text-muted-foreground">
              {state.data.recommendedPages.map((item, i) => (
                <li key={i}>{item}</li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="ml-4 list-disc text-muted-foreground">
              {state.data.uxRecommendations.map((item, i) => (
                <li key={i}>{item}</li>
              ))}
            </ul>
          </div>
